...
set "NOTEBOOK=experiment_2026_08_15_nonlinear_influence_structured_topology_discovery.ipynb"
set "RUNNER=run_ipynb_cells.py"
set "LOG_DIR=nonlinear_influence_structured_topology_discovery_logs"
...
echo Starting three nonlinear-influence shards...

for %%S in (0 1 2) do (
  start "nonlinear-influence-shard-%%S" /b cmd /v:on /c ^
    "set NLI_DISCOVERY_SHARD_ID=%%S&& set NLI_DISCOVERY_TORCH_THREADS=%THREADS_PER_SHARD%&& set MPLBACKEND=Agg&& %PYTHON_CMD% %RUNNER% %NOTEBOOK% > %LOG_DIR%\shard_%%S.log 2>&1 && (echo success> %LOG_DIR%\shard_%%S.done) || (echo failed> %LOG_DIR%\shard_%%S.failed)"
)
...
echo Open experiment_2026_08_15_nonlinear_influence_structured_topology_discovery.ipynb normally and Run All with the shard variable unset.
popd
exit /b 0
