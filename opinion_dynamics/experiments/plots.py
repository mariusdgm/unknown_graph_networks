...
def legend_outside(ax, *, loc="center left", anchor=(1.02, 0.5), ncol=1, frameon=True):
    """Place a normal legend outside the axes without covering data."""
...
def plot_mean_baseline_comparison(mean_series, title):
    """
    Mean/baseline comparison with no forced y-limits.

    Legend stays outside the plot; distinct markers/linestyles make overlaps readable.
    """
...
def build_augmented_campaign_trajectory(
    ...
    *,
    ...
    """
    Build a fine-grained trajectory for plotting impulse-control campaigns.

    Includes, per campaign:
      1. boundary/pre-impulse state at k*t_campaign
      2. immediate post-impulse state at k*t_campaign + eps_t
      3. propagation states from info['intermediate_states'] at t_s spacing
      4. next boundary at (k+1)*t_campaign
    """
    boundary_states = np.asarray(boundary_states, dtype=float)
    actions = np.asarray(actions, dtype=float)
    dt = float(dt)
    t_campaign = float(t_campaign)
    eps_t = float(1e-3 * dt if eps_t is None else eps_t)

    X_all, T_all = [], []
    K = min(actions.shape[0], len(intermediate_states_list), boundary_states.shape[0] - 1)

    for k in range(K):
        t0 = k * t_campaign
        x_pre = np.asarray(boundary_states[k], dtype=float)
        u = np.asarray(actions[k], dtype=float)
        x_post = u * float(desired_opinion) + (1.0 - u) * x_pre

        X_all.append(x_pre.copy())
        T_all.append(t0)
        X_all.append(x_post.copy())
        T_all.append(t0 + eps_t)

        inter = intermediate_states_list[k]
        if inter is not None:
            inter = np.asarray(inter, dtype=float)
            # inter[0] is usually the post-impulse state. Skip it because we already inserted x_post.
            for m in range(1, inter.shape[0]):
                X_all.append(inter[m].copy())
                T_all.append(t0 + m * dt)

        X_all.append(np.asarray(boundary_states[k + 1], dtype=float).copy())
        T_all.append(t0 + t_campaign)

    return np.asarray(X_all, dtype=float), np.asarray(T_all, dtype=float)


def plot_amount_metric(df, metric, *, ylabel=None, title=None, by="dynamics", x="trial_count"):
    """Plot mean ± one standard deviation for an amount-of-data metric."""
    if metric not in df.columns:
        print(f"Skipping {metric}: not in df_learning")
        return None
    dfx = df.copy()
...
# =============================================================================
# Source-of-truth notebook plotting helpers
# =============================================================================
import pandas as pd
from .metrics import add_trial_count_column, aggregate_learning_curve
...
# Public alias used by other modules/notebooks.
legend_outside = _legend_outside
